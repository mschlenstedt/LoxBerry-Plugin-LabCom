...
if( $q->{ajax} ) {
	...
	# Save LabCom Settings
	if( $q->{ajax} eq "savelabcom" ) {
		LOGINF "P$$ savelabcom: savelabcom was called.";
		$response{error} = &savelabcom();
		print JSON->new->canonical(1)->encode(\%response);
	}
...
##########################################################################
	...
	$q->{form} = "labcom" if !$q->{form};

	if ($q->{form} eq "labcom") { &form_labcom() }
...
sub form_print
{
	...
	$navbar{10}{Name} = "$L{'COMMON.LABEL_LABCOM'}";
	$navbar{10}{URL} = 'index.cgi?form=labcom';
	$navbar{10}{active} = 1 if $q->{form} eq "labcom";
